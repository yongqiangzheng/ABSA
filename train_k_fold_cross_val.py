...
class Instructor:
    ...
    def run(self):
        ...
        splittedsets = random_split(self.trainset, tuple([valset_len] * (self.opt.cross_val_fold - 1) + [
        ...
        for fid in range(self.opt.cross_val_fold):
            ...
            trainset = ConcatDataset(
                [x for i, x in enumerate(splittedsets) if i != fid])
        ...
        mean_test_acc, mean_test_f1 = numpy.mean(
            all_test_acc), numpy.mean(all_test_f1)
        logger.info('>' * 100)
...
def main():
    ...
    parser.add_argument('--cross_val_fold', default=10,
                        type=int, help='k-fold cross validation')
